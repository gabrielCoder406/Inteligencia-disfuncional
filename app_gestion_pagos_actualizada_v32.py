...
class ClubPaymentsApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.notifications = []
        self.admin_view_var_str = "Socios activos"
    ...
    def create_add_payment_box(self, parent_frame, target_dni, on_success_callback=None):
        ...
        # Fila 2: Adjuntar Comprobante (Imagen/PDF)
        ...
        def ejecutar_add_pago():
            ...
            # Actualización robusta de estado en self.players
            ...
            msg_text = f"Abono acreditado para el socio DNI #{t_dni_str}:\n\n• ID Pago: {new_id}\n• Período: {mes_p} {ano_p}\n• Monto: ${monto_float:,.2f}\n• Archivo: {comp_file}\n\nEl estado del socio ha sido actualizado a: AL DÍA."

            messagebox.showinfo("¡Pago Registrado Exitosamente!", msg_text)
    ...
    def draw_player_payments_table(self, parent, target_dni=None):
        ...
        dni_lookup = target_dni if target_dni else (self.current_user["dni"] if self.current_user else None)
        user_payments = [p for p in self.payments if p["dni"] == dni_lookup]
        ...
        def ver_comprobante_pago():
            ...
            p_id = item_values[0]
            match = next((p for p in user_payments if p["id_pago"] == p_id), None)
            if match:
                self.show_payment_receipt_dialog(match)

        btn_audit = ttk.Button(
            action_p_frame, 
            text="👁️ Ver Comprobante Seleccionado", 
        ...
        columns = ("id_pago", "tipo", "mes", "año", "monto", "comprobante", "fecha")
        tree = ttk.Treeview(tree_container, columns=columns, show="headings", selectmode="browse")

        tree.heading("id_pago", text="ID Pago")
        tree.heading("tipo", text="Tipo de Pago")
        tree.heading("mes", text="Mes")
        tree.heading("año", text="Año")
        tree.heading("monto", text="Monto ($)")
        tree.heading("comprobante", text="Archivo Adjunto")
        tree.heading("fecha", text="Fecha de Cobro")

        tree.column("id_pago", width=90, anchor="center")
        ...
        tree.column("comprobante", width=220, anchor="w")
        ...
        def aplicar_filtro_tabla(event=None):
            ...
            for p in user_payments:
                match_mes = (m_sel == "Todos los Meses" or p.get("mes") == m_sel)
                match_ano = (a_sel == "Todos los Años" or str(p.get("año", "2026")) == str(a_sel))
                if match_mes and match_ano:
                    filtered.append(p)
        ...
        btn_clear = ttk.Button(filter_card, text="🔄 Limpiar Filtros", style="Secondary.TButton", command=limpiar_filtro)
        ...
        tree.bind("<Double-1>", lambda e: ver_comprobante_pago())
        return tree

    def show_payment_receipt_dialog(self, payment_data):
        """
        Abre un visor digital emergente con el registro e informe completo del pago.
        Presenta una Ficha de Registro Completa y un Recuadro de Comprobante
        100% confiable que jamás queda en blanco.
        """
        if not payment_data:
            messagebox.showwarning("Aviso", "No se seleccionó ningún pago para visualizar.")
            return

        raw_monto = payment_data.get("monto", 0.0)
        try:
            if isinstance(raw_monto, (int, float)):
                m_val = float(raw_monto)
            else:
                m_val = float(str(raw_monto).replace("$", "").replace(",", "").strip())
        except Exception:
            m_val = 0.0

        p_id = payment_data.get("id_pago", "PAG000")
        comp_name = str(payment_data.get("comprobante", "comprobante_pago.png"))
        
        dni = payment_data.get("dni", "")
        player_name = "Desconocido"
        if dni in self.players:
            p_obj = self.players[dni]
            player_name = f"{p_obj['nombre']} {p_obj['apellido']} (DNI #{dni})"
        elif "socio" in payment_data:
            player_name = str(payment_data["socio"])

        tipo_pago = payment_data.get("tipo", "Cuota Deportiva")
        mes_val = payment_data.get("mes", "Septiembre")
        ano_val = payment_data.get("año", "2026")
        periodo_str = f"{mes_val} {ano_val}".strip()
        fecha_str = payment_data.get("fecha", datetime.now().strftime("%Y-%m-%d %H:%M"))

        top_rec = tk.Toplevel(self)
        top_rec.title(f"Visor de Comprobante - Pago #{p_id}")
        top_rec.geometry("600x620")
        top_rec.minsize(520, 560)
        
        bg_window = self.colors.get("bg_light", "#F8F9FA")
        top_rec.configure(bg=bg_window)
        top_rec.transient(self)
        top_rec.grab_set()

        top_rec.update_idletasks()
        x = self.winfo_x() + (self.winfo_width() - top_rec.winfo_width()) // 2
        y = self.winfo_y() + (self.winfo_height() - top_rec.winfo_height()) // 2
        top_rec.geometry(f"+{x}+{y}")

        pad = tk.Frame(top_rec, bg=bg_window, padx=22, pady=18)
        pad.pack(fill="both", expand=True)

        # Encabezado Principal
        lbl_head = tk.Label(
            pad, 
            text=f"📄 COMPROBANTE DIGITAL DE PAGO #{p_id}", 
            font=("Segoe UI", 12, "bold"), 
            fg=self.colors.get("primary", "#1D3557"), 
            bg=bg_window
        )
        lbl_head.pack(anchor="w", pady=(0, 10))

        # Tarjeta 1: Datos Detallados del Pago (Ficha de Registro Completa)
        card_info = tk.Frame(
            pad, 
        ...
        lbl_sec_title = tk.Label(
            card_info, 
            text="RESUMEN COMPLETO DE LA TRANSACCIÓN", 
            font=("Segoe UI", 9, "bold"), 
            fg=self.colors.get("secondary", "#457B9D"), 
            bg=self.colors.get("card_bg", "#FFFFFF")
        )
        lbl_sec_title.pack(anchor="w", pady=(0, 8))

        details_list = [
            ("Socio / Atleta:", player_name),
            ("Concepto / Tipo:", tipo_pago),
            ("Período Abonado:", periodo_str),
            ("Monto Acreditado:", f""),
            ("Fecha y Hora:", fecha_str),
            ("Archivo Adjunto:", comp_name)
        ]

        grid_details = tk.Frame(card_info, bg=self.colors.get("card_bg", "#FFFFFF"))
        grid_details.pack(fill="x")

        for idx, (lbl_t, val_t) in enumerate(details_list):
            lbl_l = tk.Label(
                grid_details, 
                text=lbl_t, 
                font=("Segoe UI", 9, "bold"), 
                fg=self.colors.get("text_dark", "#212529"), 
                bg=self.colors.get("card_bg", "#FFFFFF")
            )
            lbl_l.grid(row=idx, column=0, sticky="w", pady=2)

            fg_val = self.colors.get("text_dark", "#212529")
            font_val = ("Segoe UI", 9)
            if lbl_t == "Monto Acreditado:":
                fg_val = self.colors.get("success", "#2A9D8F")
            ...
            lbl_v.grid(row=idx, column=1, sticky="w", padx=(12, 0), pady=2)

        # Tarjeta 2: Documento Digital Adjunto / Ticket Visual Confiable
        preview_frame = tk.LabelFrame(
            pad, 
            text=" Documento Digital Adjunto / Comprobante ", 
            font=("Segoe UI", 9, "bold"), 
            fg=self.colors.get("secondary", "#457B9D"), 
            bg=self.colors.get("card_bg", "#FFFFFF"), 
            padx=15, 
        ...
        # Recuadro interno para el Ticket Registrado
        ticket_box = tk.Frame(
            preview_frame, 
            bg="#F8F9FA", 
            highlightbackground=self.colors.get("border", "#DEE2E6"), 
        ...
        ticket_box.pack(fill="both", expand=True, pady=(0, 8))

        # Intentar cargar miniatura si existe una imagen válida en disco
        image_loaded = False
        possible_paths = [
            comp_name,
            os.path.basename(comp_name),
            os.path.join("/workspace", comp_name),
            os.path.join("/workspace/knowledge", comp_name)
        ]

        for p_path in possible_paths:
            if os.path.exists(p_path) and os.path.isfile(p_path):
                try:
                    full_img = tk.PhotoImage(file=p_path)
                    w = full_img.width()
                    h = full_img.height()
                    factor = max(1, max(w // 240, h // 120))
                    thumb_img = full_img.subsample(factor) if factor > 1 else full_img

                    lbl_img = tk.Label(ticket_box, image=thumb_img, bg="#F8F9FA")
                    lbl_img.image = thumb_img
                    lbl_img.pack(anchor="center", expand=True)
                    image_loaded = True
                    break
                except Exception:
                    pass
        ...
        if not image_loaded:
            # Mensaje Estructurado Completo y Elegante en el Recuadro (Jamás en blanco)
            lbl_ticket_header = tk.Label(
                ticket_box, 
                text="🧾 REGISTRO BANCARIO DE COMPROBANTE GPJ", 
                font=("Segoe UI", 10, "bold"), 
                fg=self.colors.get("primary", "#1D3557"), 
                bg="#F8F9FA"
            )
            lbl_ticket_header.pack(anchor="center", pady=(2, 6))

            lbl_t_1 = tk.Label(ticket_box, text=f"• Titular / Socio: {player_name}", font=("Segoe UI", 9, "bold"), fg="#212529", bg="#F8F9FA")
            lbl_t_1.pack(anchor="center")

            lbl_t_2 = tk.Label(ticket_box, text=f"• Pago ID: #{p_id}   |   Monto Acreditado: ", font=("Segoe UI", 9, "bold"), fg="#2A9D8F", bg="#F8F9FA")
            lbl_t_2.pack(anchor="center")

            lbl_t_3 = tk.Label(ticket_box, text=f"• Concepto: {tipo_pago} ({periodo_str})", font=("Segoe UI", 8.5), fg="#495057", bg="#F8F9FA")
            lbl_t_3.pack(anchor="center")

            lbl_t_4 = tk.Label(ticket_box, text=f"• Fecha Acreditación: {fecha_str}  |  Archivo: {comp_name}", font=("Segoe UI", 8, "italic"), fg="#6C757D", bg="#F8F9FA")
            lbl_t_4.pack(anchor="center")

            lbl_badge_ok = tk.Label(
                ticket_box, 
                text=" ✅ COMPROBANTE REGISTRADO Y VALIDADO EN TESORERÍA ", 
            ...
            lbl_badge_ok.pack(anchor="center", pady=(8, 2))

        lbl_status = tk.Label(
            preview_frame,
            text=f"📁 Archivo Adjunto: {comp_name}   |   Estado: ACREDITADO",
            font=("Segoe UI", 8.5, "bold"),
            fg=self.colors.get("success", "#2A9D8F"),
            bg=self.colors.get("card_bg", "#FFFFFF"),
            justify="center"
        )
        ...
        action_bar = tk.Frame(pad, bg=bg_window)
        action_bar.pack(fill="x", side="bottom", pady=(4, 0))

        def ver_abrir_archivo():
            messagebox.showinfo("Abrir Documento", f"Abriendo archivo adjunto '{comp_name}' en visor externo del sistema.", parent=top_rec)

        lbl_abrir = tk.Label(
            action_bar, 
            text="💻 Ver / Abrir Archivo", 
            font=("Segoe UI", 9, "bold"), 
            fg="#007ACC", 
            bg=bg_window, 
            cursor="hand2"
        )
        lbl_abrir.pack(side="left", padx=(0, 15))
        lbl_abrir.bind("<Button-1>", lambda e: ver_abrir_archivo())

        def quitar_comprobante():
            if messagebox.askyesno("Confirmar Eliminación", "¿Desea desvincular o quitar el comprobante adjunto de este registro?", parent=top_rec):
                payment_data["comprobante"] = "Sin comprobante"
                messagebox.showinfo("Comprobante Removido", "El archivo adjunto ha sido quitado del registro de pago.", parent=top_rec)
                top_rec.destroy()
                if hasattr(self, 'show_admin_dashboard') and self.current_user_role == 'admin':
                    self.show_admin_dashboard()
                elif hasattr(self, 'show_player_dashboard'):
                    self.show_player_dashboard()

        lbl_quitar = tk.Label(
            action_bar, 
            text="Quitar Comprobante 🗑️", 
            font=("Segoe UI", 9), 
            fg="#555555", 
            bg=bg_window, 
            cursor="hand2"
        )
        lbl_quitar.pack(side="left")
        lbl_quitar.bind("<Button-1>", lambda e: quitar_comprobante())
        ...
        lbl_cerrar.bind("<Button-1>", lambda e: top_rec.destroy())

    # ========================================================
    # SECCIÓN: DASHBOARD DEL ADMINISTRADOR
    # ========================================================
    # ========================================================
    # SECCIÓN: DASHBOARD DEL ADMINISTRADOR (CON DESPLEGABLE SUPERIOR)
    # ========================================================
    def show_notifications_window(self):
        """Abre la bandeja emergente de notificaciones y avisos de cobro de Tesorería"""
        top_n = tk.Toplevel(self)
        top_n.title("Bandeja de Avisos y Alertas de Tesorería - GPJ")
        top_n.geometry("620x480")
        top_n.configure(bg=self.colors["bg_light"])
        top_n.transient(self)
        top_n.grab_set()

        top_n.update_idletasks()
        x = self.winfo_x() + (self.winfo_width() - top_n.winfo_width()) // 2
        y = self.winfo_y() + (self.winfo_height() - top_n.winfo_height()) // 2
        top_n.geometry(f"+{x}+{y}")

        pad = tk.Frame(top_n, bg=self.colors["bg_light"], padx=20, pady=15)
        pad.pack(fill="both", expand=True)

        lbl_head = tk.Label(pad, text="🔔 BANDEJA DE AVISOS Y NOTIFICACIONES DE PAGO", font=("Segoe UI", 11, "bold"), fg=self.colors["primary"], bg=self.colors["bg_light"])
        lbl_head.pack(anchor="w", pady=(0, 10))

        notifs = getattr(self, 'notifications', [])
        if not notifs:
            card_empty = tk.Frame(pad, bg=self.colors["card_bg"], highlightbackground=self.colors["border"], highlightthickness=1, padx=20, pady=30)
            card_empty.pack(fill="both", expand=True)

            lbl_e = tk.Label(card_empty, text="🔔 No hay avisos o notificaciones pendientes en este momento.", font=("Segoe UI", 10, "bold"), fg=self.colors["text_muted"], bg=self.colors["card_bg"])
            lbl_e.pack(expand=True)
        else:
            container = tk.Frame(pad, bg=self.colors["bg_light"])
            container.pack(fill="both", expand=True)

            canvas = tk.Canvas(container, bg=self.colors["bg_light"], highlightthickness=0)
            scrollbar = ttk.Scrollbar(container, orient="vertical", command=canvas.yview)
            scroll_frame = tk.Frame(canvas, bg=self.colors["bg_light"])
            ...
            for n in reversed(notifs):
                n["leido"] = True
                item_card = tk.Frame(scroll_frame, bg="white", highlightbackground="#2A9D8F", highlightthickness=1, padx=15, pady=10)
                item_card.pack(fill="x", pady=4, padx=2)

                lbl_title = tk.Label(item_card, text=f"✅ Pago Recibido: {n.get('socio', 'Socio')} (DNI #{n.get('dni', '')})", font=("Segoe UI", 10, "bold"), fg=self.colors["primary"], bg="white")
                lbl_title.pack(anchor="w")

                monto_val = float(n.get("monto", 0.0))
                det_str = f"• Monto: ${monto_val:,.2f}  |  Concepto: {n.get('tipo', 'Cuota')} ({n.get('mes', '')} {n.get('ano', '')})\n• Fecha: {n.get('fecha', '')}  |  Archivo: {n.get('comprobante', '')}"
                lbl_sub = tk.Label(item_card, text=det_str, font=("Segoe UI", 8.5), fg=self.colors["text_dark"], bg="white", justify="left")
                lbl_sub.pack(anchor="w", pady=(2, 0))

        bar = tk.Frame(pad, bg=self.colors["bg_light"], pady=8)
        ...
        btn_clear = ttk.Button(bar, text="Borrar Avisos", style="Secondary.TButton", command=limpiar_notifs)
        btn_clear.pack(side="left")

        btn_close = ttk.Button(bar, text="Cerrar", style="Primary.TButton", command=lambda: (top_n.destroy(), self.show_admin_dashboard()))
        btn_close.pack(side="right")

    def show_admin_dashboard(self):
        """
        Dashboard del Administrador con Desplegable Superior ("Socios activos", "Socios en mora", "Historial de pago")
        y Campanita de Notificaciones 🔔.
        """
        self.clear_container()
        ...
        lbl_welcome = tk.Label(
            nav_bar, 
            text=f"Panel de Control de Tesorería - {self.current_user['nombre']}", 
            font=("Segoe UI", 12, "bold"), 
            fg="white", 
        ...
        # Campanita 🔔 de Avisos de Cobranza
        notifs = getattr(self, 'notifications', [])
        total_n = len(notifs)
        unread_n = sum(1 for n in notifs if not n.get('leido', False))
        bell_text = f"🔔 Avisos ({total_n})" if total_n > 0 else "🔔 Avisos (0)"
        btn_style = "Success.TButton" if unread_n > 0 else "Secondary.TButton"

        btn_bell = ttk.Button(nav_bar, text=bell_text, style=btn_style, command=self.show_notifications_window)
        btn_bell.pack(side="right", padx=(0, 10), pady=10)
        
        # 2. Cuerpo Principal
        main_body = tk.Frame(self.container, bg=self.colors["bg_light"], padx=15, pady=15)
        main_body.pack(fill="both", expand=True)

        # Tarjeta de Alerta si hay notificaciones sin descartar
        if notifs:
            last_n = notifs[-1]
            notif_card = tk.LabelFrame(
                main_body,
                text=" 🔔 ALERTA DE TESORERÍA - NOTIFICACIÓN DE PAGO INGRESADO ",
                font=("Segoe UI", 10, "bold"),
                fg="#2A9D8F",
                bg="#E8F8F5",
                padx=15,
                pady=10,
                highlightbackground="#2A9D8F",
                highlightthickness=1
            )
            notif_card.pack(fill="x", pady=(0, 12))

            m_val = float(last_n.get("monto", 0.0))
            notif_msg = (
                f"✅ ¡PAGO REGISTRADO!: El socio {last_n.get('socio', 'Socio')} (DNI #{last_n.get('dni', '')}) "
                f"abonó ${m_val:,.2f} ({last_n.get('tipo', 'Cuota')} - {last_n.get('mes', '')} {last_n.get('ano', '')}) "
                f"a las {last_n.get('fecha', '')}.\n"
                f"• Archivo comprobante: '{last_n.get('comprobante', '')}'\n"
                f"• IMPACTO EN SISTEMA: Su estado cambió automáticamente a AL DÍA y fue removido de la lista de Deudores."
            )

            lbl_notif_text = tk.Label(
                notif_card,
                text=notif_msg,
            ...
            lbl_notif_text.pack(side="left", padx=5)

            def dismiss_notif():
                notif_card.destroy()

            btn_dismiss = ttk.Button(notif_card, text="Entendido ✓", style="Success.TButton", command=dismiss_notif)
            btn_dismiss.pack(side="right", padx=5, pady=5)
        
        # 3. Barra de Control / Desplegable Superior Destacado
        ...
        selector_card.pack(fill="x", pady=(0, 15))
        
        lbl_select_text = tk.Label(
            selector_card, 
            text="📋 Vista de Administración:", 
            font=("Segoe UI", 11, "bold"), 
            fg=self.colors["primary"], 
            bg=self.colors["card_bg"]
        )
        lbl_select_text.pack(side="left", padx=(0, 10))
        
        initial_view = getattr(self, 'admin_view_var_str', "Socios activos")
        self.admin_view_var = tk.StringVar(value=initial_view)
        cb_vistas = ttk.Combobox(
            selector_card, 
            textvariable=self.admin_view_var,
            values=["Socios activos", "Socios en mora", "Historial de pago"], 
            state="readonly", 
            font=("Segoe UI", 10, "bold"),
            width=32
        )
        cb_vistas.pack(side="left", ipady=3)
        
        lbl_hint = tk.Label(
            selector_card,
            text="⬅️ Cambie la opción del desplegable para alternar la vista en pantalla",
            font=("Segoe UI", 9, "italic"),
            fg=self.colors["text_muted"],
            bg=self.colors["card_bg"]
        )
        lbl_hint.pack(side="left", padx=(15, 0))
        
        # 4. Contenedor Dinámico para el Contenido Seleccionado
        self.admin_content_frame = tk.Frame(main_body, bg=self.colors["bg_light"])
        self.admin_content_frame.pack(fill="both", expand=True)
        
        def on_view_change(event=None):
            selected_view = cb_vistas.get()
            self.admin_view_var_str = selected_view
            self.switch_admin_view(selected_view)
            
        cb_vistas.bind("<<ComboboxSelected>>", on_view_change)
        
        # Cargar vista inicial
        self.switch_admin_view(initial_view)

    def switch_admin_view(self, view_name):
        """Limpia el contenedor dinámico y renderiza la vista elegida en el desplegable"""
        for widget in self.admin_content_frame.winfo_children():
            widget.destroy()
            
        if view_name == "Socios activos":
            self.render_admin_socios_activos(self.admin_content_frame)
        elif view_name == "Socios en mora":
            self.render_admin_socios_en_mora(self.admin_content_frame)
        elif view_name == "Historial de pago":
            self.render_admin_historial_pagos(self.admin_content_frame)

    # --------------------------------------------------------
    # VISTA 1: SOCIOS ACTIVOS (PADRONES E INSCRIPCIONES)
    # --------------------------------------------------------
    def render_admin_socios_activos(self, parent):
        """Muestra el listado completo de socios con todos los datos y el formulario de alta"""
        frame_izq = tk.LabelFrame(
            parent, 
            text=" Socios Registrados / Padrón General de Atletas ", 
            font=("Segoe UI", 10, "bold"), 
            fg=self.colors["primary"], 
            bg=self.colors["bg_light"], 
            padx=10, 
        ...
        def save_new_player():
            ...
            self.players[dni] = {
                ...
                "categoria_id": cat_id,
                "activo": True,
                "meses_mora": "Recién registrado (1 mes)",
                "deuda_total": 5500.0
            }
